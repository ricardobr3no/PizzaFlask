...
def home():
    ...
    quantidade_itens = sum(item.quantidade for item in itens_carrinho)
...
def adicionar_carrinho(item_id):
    ...
    cliente_id = session["cliente_id"]
    produto = item_ctrl.get_by_id(item_id)
    ...
    item_pedido = ItemPedido.query.filter_by(
        pedido_id=carrinho.id, item_id=produto.id
    ).first()

    if item_pedido:
        item_pedido.quantidade += 1
        item_pedido.subtotal = item_pedido.quantidade * produto.preco
        db.session.commit()
    else:
...
def finalizar_pedido():
    ...
    if carrinho and carrinho.itens:
        pedido_ctrl.update(
            carrinho.id, {"status": Status.DISPONIVEL.value, "data": datetime.now}
        )
